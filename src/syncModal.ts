...
export class ExamAppSyncModal extends Modal {
	...
	onOpen() {
		...
		let lastSyncText = 'Son güncelleme: Henüz yapılmadı';
		if (this.plugin.settings.lastSyncTimestamp) {
			try {
				...
				lastSyncText = `Son güncelleme: ${dateStr}${modeStr ? ` (${modeStr})` : ''}`;
			} catch (e) {
				lastSyncText = `Son güncelleme: ${this.plugin.settings.lastSyncTimestamp}`;
			}
		}
